...
describe("swap eligibility", () => {
  ...
  it("auto-approves an available, qualified recipient", () => {
    const sam = db.employees.find((e) => e.id === "emp-sam")!; // available Mon 8-17, qualified
    const ev = evaluateSwap({
      shift, recipient: sam, position,
      recipientPatterns: db.availability.filter((p) => p.employeeId === sam.id),
      recipientLeave: [], leaveTypes: db.leaveTypes,
      recipientShiftsThatDay: [], policy: defaultCaliforniaPolicy("non_exempt_staff"),
      positions: db.positions, cutoffOk: true, weeklyMinutesSoFar: 0,
    });
  ...
  it("routes to review when recipient is unavailable", () => {
    const jordan = db.employees.find((e) => e.id === "emp-jordan")!; // Mon available 13-20 preferred — actually available
    // Force unavailability: evaluate a morning shift Jordan cannot take
    const morning = { ...shift, start: 540, end: 660 }; // 9-11, Jordan unavailable
    const ev = evaluateSwap({
      shift: morning, recipient: jordan, position,
      recipientPatterns: db.availability.filter((p) => p.employeeId === jordan.id),
      recipientLeave: [], leaveTypes: db.leaveTypes,
      recipientShiftsThatDay: [], policy: defaultCaliforniaPolicy("student_worker"),
      positions: db.positions, cutoffOk: true, weeklyMinutesSoFar: 0,
    });
  ...
  it("blocks a locked shift", () => {
    const locked = db.shifts.find((s) => s.id === "shift-sam-mon")!; // locked
    const avery = db.employees.find((e) => e.id === "emp-avery")!;
    const ev = evaluateSwap({
      shift: locked, recipient: avery, position,
  ...
  it("blocks after the cutoff", () => {
    ...
    const ev = evaluateSwap({
      ...
      recipientLeave: [], leaveTypes: db.leaveTypes,
      recipientShiftsThatDay: [], policy: defaultCaliforniaPolicy("non_exempt_staff"),
      positions: db.positions, cutoffOk: false, weeklyMinutesSoFar: 0,
    });
    expect(ev.autoApprovable).toBe(false);
