...
/** Deterministic reference clock for seeded records (keeps engines pure). */
export const SEED_NOW = "2026-07-13T08:00:00.000Z";
/** Monday of the seeded operating week. */
...
function operatingHours(): OperatingHours[] {
  // Mon–Thu 08:00–20:00, Fri 08:00–18:00, Sat 10:00–17:00, Sun closed.
  const weekly: Record<number, { start: number; end: number }[]> = {
    0: [],
    1: [{ start: T("08:00"), end: T("20:00") }],
    2: [{ start: T("08:00"), end: T("20:00") }],
    3: [{ start: T("08:00"), end: T("20:00") }],
    4: [{ start: T("08:00"), end: T("20:00") }],
    5: [{ start: T("08:00"), end: T("18:00") }],
    6: [{ start: T("10:00"), end: T("17:00") }],
  };
...
function tasks(): Task[] {
  const common = {
    ...
    applicableLocationIds: ["loc-main", "loc-desk", "loc-reading"],
  ...
  return [
    ...
    { ...common, id: "task-opening", name: "Opening duties", category: "Operations", colorToken: "task-warn", icon: "sunrise", estimatedMinutes: 30, openingDependency: true, requiresAcknowledgement: true, checklist: ["Unlock service points", "Power on public terminals", "Check overnight holds"], order: 4 },
    { ...common, id: "task-closing", name: "Closing duties", category: "Operations", colorToken: "task-warn", icon: "sunset", estimatedMinutes: 30, closingDependency: true, requiresAcknowledgement: true, checklist: ["Sweep reading room", "Secure service points", "Set alarm"], order: 5 },
  ];
}
...
function leaveTypes(): LeaveType[] {
  ...
  return [
    { ...c, id: "lt-sick", name: "Sick", paid: true, approvalRequired: false, countsAgainstBalance: true, visibility: "manager", blocksScheduling: true, requiresNote: false },
    { ...c, id: "lt-vacation", name: "Vacation", paid: true, approvalRequired: true, countsAgainstBalance: true, visibility: "team_generic", blocksScheduling: true, requiresNote: false },
    { ...c, id: "lt-pto", name: "PTO", paid: true, approvalRequired: true, countsAgainstBalance: true, visibility: "team_generic", blocksScheduling: true, requiresNote: false },
    { ...c, id: "lt-rdo", name: "Regular day off", paid: false, approvalRequired: false, countsAgainstBalance: false, visibility: "team_generic", blocksScheduling: true, requiresNote: false },
    { ...c, id: "lt-floating", name: "Floating holiday", paid: true, approvalRequired: true, countsAgainstBalance: true, visibility: "team_generic", blocksScheduling: true, requiresNote: false },
    { ...c, id: "lt-holiday", name: "Holiday", paid: true, approvalRequired: false, countsAgainstBalance: false, visibility: "team_generic", blocksScheduling: true, requiresNote: false },
  ];
}

/** Recurring availability for a fictional employee, weekdays 1–5. */
function weekdayAvailability(
  id: string,
  employeeId: string,
  windows: { days: number[]; start: string; end: string; kind: "preferred" | "available" | "unavailable" }[],
): AvailabilityPattern {
  const blocks = windows.flatMap((w) =>
    w.days.map((weekday) => ({ weekday, start: T(w.start), end: T(w.end), kind: w.kind })),
  );
  return { id, employeeId, label: "Summer term", blocks, updatedBy: employeeId, updatedAt: SEED_NOW };
}

export function buildSeed(): Database {
  const db = emptyDatabase();

  // --- Users & manager profiles from the five bootstrap administrators ---
  const adminIds = ["admin-whuggins", "admin-cadena", "admin-blalfaro", "admin-gwilson", "admin-bwilli"];
  BOOTSTRAP_ADMINS.forEach((admin, i) => {
    const id = adminIds[i];
    db.users.push(user(id, admin.name, admin.email, [{ role: "SUPER_ADMIN" }, { role: "MANAGER" }]));
    db.employees.push(
      baseProfile({
        id,
        ...
        qualifiedPositionIds: ["pos-desk", "pos-admin", "pos-project", "pos-meetings", "pos-learning"],
      }),
    );
  });

  // --- Fictional development employees ---
  ...
  // A user awaiting administrator approval (authenticated, Stanford domain, no invite).
  db.users.push({
    id: "pending-taylor",
    email: "taylor.nguyen@stanford.edu",
  ...
  // An outstanding invitation with a signed, expiring, single-use token.
  db.invitations.push({
    id: "invite-1",
    email: "casey.brooks@law.stanford.edu",
    token: "seed-invite-token-abc123",
    role: "EMPLOYEE",
    managerId: "admin-cadena",
    departmentId: "dept-access",
    expiresAt: addDays(SEED_WEEK_START, 14) + "T00:00:00.000Z",
    revoked: false,
    createdBy: "admin-whuggins",
    createdAt: SEED_NOW,
  });

  // --- Org structure ---
  db.departments.push(
    { id: "dept-access", name: "Access Services", active: true },
    { id: "dept-collections", name: "Collections", active: true },
  );
  db.teams.push(
    { id: "team-desk", name: "Public Services", departmentId: "dept-access", active: true },
    { id: "team-stacks", name: "Stacks", departmentId: "dept-collections", active: true },
  );
  ...
  db.breakPolicies.push(defaultCaliforniaPolicy("non_exempt_staff"), {
    ...defaultCaliforniaPolicy("student_worker"),
    id: "ca-student-v1",
    name: "Student worker (California)",
  }, {
    id: "exempt-v1",
  ...
  db.availability.push(
    weekdayAvailability("avail-maya", "emp-maya", [
      { days: [1, 3], start: "09:00", end: "14:00", kind: "preferred" },
      { days: [2, 4], start: "12:00", end: "17:00", kind: "available" },
      { days: [5], start: "09:00", end: "13:00", kind: "available" },
    ]),
    weekdayAvailability("avail-jordan", "emp-jordan", [
      { days: [1, 2, 3, 4], start: "13:00", end: "20:00", kind: "preferred" },
      { days: [5], start: "13:00", end: "18:00", kind: "available" },
    ]),
    weekdayAvailability("avail-sam", "emp-sam", [
      { days: [1, 2, 3, 4, 5], start: "08:00", end: "17:00", kind: "preferred" },
    ]),
    weekdayAvailability("avail-avery", "emp-avery", [
      { days: [1, 2, 3], start: "10:00", end: "18:00", kind: "preferred" },
      { days: [4, 5], start: "10:00", end: "16:00", kind: "available" },
    ]),
    weekdayAvailability("avail-noah", "emp-noah", [
      { days: [2, 4], start: "09:00", end: "13:00", kind: "available" },
    ]),
    weekdayAvailability("avail-riley", "emp-riley", [
      { days: [3, 6], start: "10:00", end: "17:00", kind: "available" },
    ]),
  );

  // --- Schedule + coverage requirements for the seeded week ---
  db.schedules.push({
    id: "sched-week",
    name: `Week of ${SEED_WEEK_START}`,
    startDate: SEED_WEEK_START,
    endDate: addDays(SEED_WEEK_START, 6),
    status: "draft",
    version: 1,
    createdBy: "admin-whuggins",
    createdAt: SEED_NOW,
  ...
  // A few already-published example shifts so views render immediately.
  db.shifts.push(...buildExampleShifts());

  // --- One approved leave record (Avery, Thursday) ---
  db.leave.push({
    id: "leave-avery-thu",
    employeeId: "emp-avery",
    leaveTypeId: "lt-vacation",
    startDate: addDays(SEED_WEEK_START, 3),
    endDate: addDays(SEED_WEEK_START, 3),
    partialDay: false,
    status: "approved",
    enteredBy: "emp-avery",
    decidedBy: "admin-whuggins",
    createdAt: SEED_NOW,
    updatedAt: SEED_NOW,
  });

  // --- One structured manager note the engine may use ---
  db.notes.push({
    id: "note-maya-desk",
    type: "employee_preference",
    title: "Limit Maya's consecutive desk time",
    body: "Try not to place Maya at the desk for more than two consecutive hours.",
    visibility: "manager",
    priority: "normal",
    authorId: "admin-whuggins",
    employeeIds: ["emp-maya"],
    positionIds: ["pos-desk"],
    locationIds: [],
    taskIds: [],
    usableByEngine: true,
    ruleClass: "soft",
...
function buildCoverage(): CoverageRequirement[] {
  ...
  // Desk coverage split into 2-hour windows, Mon–Fri, during open hours.
  const weekdayWindows: Record<number, { start: string; end: string }[]> = {
  ...
  for (let d = 1; d <= 5; d++) {
    const date = addDays(SEED_WEEK_START, d - 1);
    for (const [i, w] of weekdayWindows[d].entries()) {
      reqs.push({
        id: `cov-${date}-desk-${i}`,
        date,
        positionId: "pos-desk",
...
function buildExampleShifts(): Shift[] {
  ...
  return [
    mk({ id: "shift-sam-mon", employeeId: "emp-sam", positionId: "pos-desk", locationId: "loc-desk", date: SEED_WEEK_START, start: T("09:00"), end: T("13:00"), taskIds: ["task-opening"], breaks: [{ kind: "rest", start: T("11:00"), end: T("11:10"), paid: true }], locked: true }),
    mk({ id: "shift-maya-mon", employeeId: "emp-maya", positionId: "pos-desk", locationId: "loc-desk", date: SEED_WEEK_START, start: T("13:00"), end: T("15:00") }),
    mk({ id: "shift-jordan-mon", employeeId: "emp-jordan", positionId: "pos-project", locationId: "loc-workroom", date: SEED_WEEK_START, start: T("15:00"), end: T("18:00"), taskIds: ["task-shelfread"], status: "draft" }),
  ];
}
