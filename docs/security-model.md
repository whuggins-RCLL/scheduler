...
Approved Stanford domains are required, but domain checks are not sufficient. Access also requires invitation, approval, or bootstrap administrator status. High-level roles are Firebase custom claims; scoped permissions are stored in Firestore. Historical records are archived, not deleted.
